...
dependencies: ["SOP_00_Skill_Lifecycle_Management.md"]
---
...
> [!IMPORTANT]
> **Watchdog 機制說明**：系統已全面採用非同步的 Watchdog Hook 機制，所有異常直接寫入 Neon DB 佇列，廢除實體輪詢腳本與本地 JSON 儲存。不再使用或依賴任何實體 `watchdog.ps1` 或 `Pending_Optimization.json`。
...
│   ├── SOP_03_Skills_Maintenance.md
│   ├── SOP_04_Data_Cleanup.md     ← 本文件
│   ├── SOP_05_System_Policies.md
│   ├── SOP_06_Handover_Manual.md
│   ├── SOP_07_Program_CPU.md      ← 已遷移為 SOP_09
│   ├── SOP_08_Project_Readme.md
│   ├── SOP_09_AutoResearch_CPU.md
│   └── reports/
...
1. **翻譯覆蓋率要求**：每項 Skill 的說明必須確認 `scripts\skill_translations.json` 中已有完整的正體中文翻譯，填寫率須達 100%。
...
| **Skills Dashboard** | **8888** | http://localhost:8888/skills_dashboard.html | 技能總覽視覺化介面 |
