...
**侷限**：「快速迭代」在硬體原型中是合理的（火箭炸了再造一個）。在涉及人命、法律、政治的領域，「快速失敗」的代價不可逆。馬斯克對FSD時間線的反覆過度承諾已嚴重損害公資訊力。
...
6. **親自下場解決最關鍵瓶頸**：不是委派，而是CEO本人到現場。產能出問題就睡工廠，程式碼有問題就自己稽核。這製造了「我比任何人都在乎」的資訊號。

7. **跨公司資源槓桿**：自家火箭發自家衛星，自家平臺跑自家AI模型，自家汽車收集自家自動駕駛資料。讓每個實體成為其他實體的客戶和資料源。

8. **激進時間線作為壓力工具**：對外承諾遠超實際可能的時間線，製造內部緊迫感。接受「喊狼來了」的資訊譽損失，換取實際交付速度的提升。
...
3. **時間線預估不可資訊**：如果用這個Skill評估專案時間線，結果需要至少乘以2-3倍才接近現實。馬斯克自己承認是「喊FSD狼來了的男孩」。
