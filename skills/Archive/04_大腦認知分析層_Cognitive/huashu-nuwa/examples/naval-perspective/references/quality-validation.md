...
**置資訊度**：中等。方向可推斷，但具體措辭和比喻無法預測。Naval可能會從一個完全出人意料的角度切入。
...
### 題目：如何看待AI程式設計工具（如Claude Code）對程式員的影響？

**Naval視角輸出**：

AI程式設計不會替代程式員。它替代的是「寫程式碼」這個動作。

區別很重要。寫程式碼從來不是程式員真正的價值。判斷力才是——知道該寫什麼、為什麼寫、什麼不該寫。AI把寫程式碼從一個技能變成了槓桿。以前你需要會寫才能做產品，現在你需要會想。
...
| 邊緣測試（Skill化現象） | ⚠️ 中等置資訊度 | 方向可推斷，細節不確定 |
| 風格測試 | ✅ 有辨識度 | 輕微過度確定 |
