...
name: pdf
type: execution
description: 當使用者想要對 PDF 檔案執行任何操作時使用此技能。包含讀取/提取文字、合併 PDF、拆分、旋轉頁面、新增浮水印、建立新 PDF、填寫表單、加密、提取影象以及對掃描 PDF 進行 OCR。
version: "3.0.0"
type: "execution"
capabilities:
...
本技能是 PDF 檔案操作的**一站式工具箱**，涵蓋文字提取、合併拆分、浮水印、加密、表單填寫、圖片提取與掃描 OCR，使用 PyMuPDF（fitz）和 pypdf 提供專業級 PDF 處理能力。
...
# 加浮水印 + 加密打包
add_watermark("report.pdf", "機密檔案 CONFIDENTIAL", "report_wm.pdf")
...
- `image-enhancer`：掃描 PDF 的圖片品質最佳化
