...
本技能是 本協作系統 生態系統的**核心建構標準**，定義如何設計、分類、登記並維護所有技能（Skills）。任何新技能的建立或既有技能的重大更新，都必須依照本檔案的規範執行。
...
2. **可觸發性 (Triggerable)**：技能的觸發條件必須清晰，讓 Agent 在正確時機自動呼叫。
...
### Step 4：執行儀錶板同步
```powershell
node "<USER_HOME>\Desktop\HH.AI_260806\.agent\scripts\refresh_skills.js"
```
...
> [!IMPORTANT]
> **禁止透過 DLP 修復指令碼批次覆蓋技能內容。** 過去曾發生指令碼將整個 SKILL.md 替換為重複 DLP 行的災難性錯誤。

1. **直接編輯** 目標 SKILL.md，保留所有原有內容
2. 確認 DLP 區塊**只出現一次**（位於檔案末尾的 `[Security]` 區段）
