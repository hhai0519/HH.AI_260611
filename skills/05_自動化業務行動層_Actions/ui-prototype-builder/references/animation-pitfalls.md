...
**踩的坑**：用 `render-video.js` 3 個程式並行錄 3 個 HTML。因為 TMP_DIR 只用 `Date.now()` 命名，3 個程式同毫秒啟動時共用同一個 tmp 目錄。最先完成的程式清理 tmp，另外兩個讀目錄時 `ENOENT`，全部崩潰。

**規則**：
- 任何多程式可能共用的臨時目錄，命名必須帶 **PID 或隨機字尾**：
  ```js
  const TMP_DIR = path.join(DIR, '.video-tmp-' + Date.now() + '-' + process.pid);
  ```
- 如果確實想多檔案並行，用 shell 的 `&` + `wait` 而不是在一個 node 指令碼裡 fork
...
- 60fps 標籤價值是**上傳平臺的演演算法識別**（Bilibili / YouTube 上 60fps 標記會優先推流），實際感知流暢度對 CSS 動畫來說提升微弱
