...
name: webapp-testing
type: execution
description: 使用 Playwright 互動和測試本地 Web 應用程式的工具包。支援驗證前端功能、偵錯 UI 行為、擷取瀏覽器螢幕截圖以及檢視瀏覽器日誌。
version: "3.0.0"
type: "execution"
capabilities:
...
本技能透過 **Playwright** 對本地 Web 應用進行即時互動測試：一鍵截圖存證、捕獲 JS 錯誤、驗證元素狀態、模擬使用者操作，提供比完整 E2E 框架更快速的即時除錯迴圈。

> **與 `playwright-automation` 的區別**：本技能用於**快速即時除錯**，無需建立測試套件；`playwright-automation` 用於建立**完整自動化測試框架**。
...
class QuickWebTester:
    ...
    async def click_and_observe(self, selector: str, wait_ms: int = 1000):
        ...
        print(f"✅ 點選完成，截圖對比：{before_screenshot} → {after_screenshot}")
