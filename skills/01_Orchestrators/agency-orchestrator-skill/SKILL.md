...
## 新增指令路由與特權豁免
1. **自動化面板映射**：將面板切換邏輯委派給 `00_Master_Menu.ps1` 內部處理。總管代理人在收到如 `$$自動化$$`、`$$LINE連線$$` 或 `$$TG連線$$` 的觸發詞時，必須執行類似 `pwsh.exe -NoProfile -ExecutionPolicy Bypass -File C:\Users\HH.AI_260726\Desktop\HH.AI_260726\00_Master_Menu.ps1 -Panel 自動化` (或對應的 `LINE橋接`, `TG橋接`) 的指令。
