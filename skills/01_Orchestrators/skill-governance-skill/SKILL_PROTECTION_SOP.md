...
> [!CAUTION]
> 以下規則為**絕對規則**，任何 AI Agent、腳本、或自動化流程**皆不得違反**。
> 修改這些規則本身也需要使用者**至少 2 次明確確認**。

### 規則 1：不刪除原則（No-Delete Policy）
- **嚴禁**刪除任何 SKILL.md 檔案
- **嚴禁**刪除任何技能資料夾
- **嚴禁**以任何方式（重命名、移動、清空）使技能失效
- 唯一例外：經使用者 **2 次以上明確確認**，且記錄原因於日誌

### 規則 2：禁止批量覆蓋（No Batch Overwrite）
- **嚴禁**使用腳本批量修改 SKILL.md 的核心內容
- **嚴禁**使用 fix_dlp.js / patch_missing_dlp.js 等自動修復腳本
...
- 此規則已寫入 refresh_skills.js 代碼中，附帶安全註釋
...
- **廢除原因**：`SKILL_TW.md` 被 DLP 洪水感染腳本覆蓋，導致 27 個技能的 Modal 內容崩潰
- **現行鐵律**：**嚴禁**建立新的 `SKILL_TW.md`；現存的 15 個 `SKILL_TW.md` 不被系統讀取，僅保留作歷史參考
- **單一真相來源**：`SKILL.md` 是唯一的技能定義文件，繁體中文在儀錶板的顯示名稱透過 `skill_translations.json` 提供
...
- **嚴重等級**：🔴 CRITICAL
- **根因**：自動化 DLP 修復腳本 (fix_dlp.js) 向 SKILL_TW.md 注入了數百行重複的 DLP 宣告文字
...
- **解決方式**：按 Ctrl+Shift+R 強制刷新，或用新檔名開啟
...
| 20260417_082532 | FINAL CLEAN | Health Guard 通過後最終確認版 |
...
> **本文件由 本協作系統 Agent 於 2026-04-17 建立**
> **修改本文件的任何內容都需要使用者 2 次以上明確確認**
