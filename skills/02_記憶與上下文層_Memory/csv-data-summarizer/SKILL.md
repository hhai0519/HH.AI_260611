...
本技能利用 **Python + pandas + matplotlib + seaborn** 全自動解析 CSV / TSV 資料，產出完整的描述性統計報告、多種分佈圖表、時間序列趨勢與相關性熱力圖，讓資料洞察在 30 秒內呈現。
...
def generate_visualizations(df: pd.DataFrame, numeric_cols, output_dir: str):
    ...
    if n_cols > 0:
        ...
        for i, col in enumerate(numeric_cols):
            ...
            axes[i][0].set_title(f'{col} 分佈', fontsize=12)
        ...
        plt.close()
        print(f"✅ 分佈圖 → {output_dir}/distributions.png")
...
| 檔案 | 內容 |
|---|---|
| `distributions.png` | 每個數值欄位的分佈圖 + 箱形圖 |
