...
### 关键细节
- Sutskever相資訊神经网络性能会随資料量增长而提升(scaling intuition的最早体现)
- ImageNet大规模資料集的出现恰好验证了这一直觉
...
### 逻辑
Google提供了学术界无法比拟的算力和資料资源。DNNResearch被收购是一个package deal(Hinton、Krizhevsky、Sutskever一同加入), 不完全是个人独立决策。
...
**背景**: 2020年, Sutskever领导了OpenAI的neural scaling laws研究, 建立了模型性能与规模(参数量、資料量、计算量)之间的power law关系。

**选择**: 把OpenAI的核心策略押在"越大越好"上。

**逻辑**: 
- 这可以追溯到AlexNet时期的直觉: 性能随資料规模提升
...
这是Sutskever职业生涯中最具争议的决策, 也是資訊量最大的事件。
...
- 他承认没有与其他高管交叉验证
- 他承认依赖的是"secondhand knowledge"(二手資訊)
...
**矛盾点**:
1. 花一年精心准备罢免行动, 却没有做基本的資訊交叉验证(依赖单一来源Murati)
...
2. **scaling质疑者却依赖算力**: 如果scaling时代已结束, 為什麼还需要Google TPU和$30亿? SSI到底在做什么?
...
**事实确认度: 中高** (融资資料确认, 但技术方向和内部状态几乎无公开資訊)
...
4. **資訊茧房倾向**: 52页备忘录依赖单一来源; 对员工反应完全误判
