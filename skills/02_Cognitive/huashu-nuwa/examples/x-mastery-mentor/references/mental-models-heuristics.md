...
> 按需加载：使用者问「為什麼这样做」「底层逻辑」「思维框架」时，或场景A/B需要深入解释时读取。
...
- 标题必答三问：给谁看？讲什么？為什麼要读？
- 参见 `writing-workshop.md > Hook改进示例` 获取before/after对比

**局限**：过度优化hook会导致标题党。内容必须兑现hook的承诺，否则长期資訊任会被消耗。
...
**Hormozi的核心洞察**：只有1%的人会自己去做，99%的人愿意付费让别人帮他们做。免费给出高价值内容 → 证明你有解决方案 → 建立資訊任和互惠 → 自然转化。
...
- 公开收入（MRR截图）、过程（功能迭代）、失败（97%專案失败）
- 核心机制：观众看着你从0到$100K MRR，产生「投资人心态」——他们希望你成功，主动传播
- 分享什么：MRR里程碑、功能上线、失败复盘、技术栈决策、使用者反馈
- 不分享什么：精确获客成本、客户个人資訊、核心实现细节
...
- **行动**：发一条tweet测试核心观点，資料好再扩展

### 2. Hook占50%创作时间 ← 模型2应用
写10-15个版本选最好的。标题必答：给谁看？讲什么？為什麼要读？参见「Hook改进示例」。
- **触发**：开始写任何内容时
...
- **触发**：内容策略规划时
- **Sahil資料**：newsletter广告月收入$70K+，全部再投入增长

### 7. 4A选题矩阵
一个话题 × 4角度 = 无限选题：
- **Actionable**：教读者怎么做（Tips/Guides/How-to）
- **Analytical**：用資料支撑（Stats/Trends/Frameworks）
