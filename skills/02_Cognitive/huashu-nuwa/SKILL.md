...
使用者说「就做XX」没有更多資訊 → 默认全面画像 + 思维顾问 + 无本地语料（走网络搜索），直接推进。
使用者提供了本地语料 → 标记为**本地语料模式**，Phase 1的采集策略会相应调整。
...
使用者不知道该蒸馏谁，只有需求或困惑。这时女娲的工作是**从需求反推最合适的蒸馏对象**。

#### Step 1: 需求定位

通过1-2个追问，定位使用者的核心需求维度：
...
| 设计与产品 | 「使用者体验差」「产品没特色」「不知道做减法」 | 极简主义、使用者心理模型、约束即创意 |
| 幽默与表达力 | 「说话没意思」「想让内容更有趣」「太严肃了」 | 荒诞对比、预期违背、自嘲式权威 |

追问原则：
- 最多问2轮，不要变成问卷调查
- 如果使用者已经表达得足够清晰，不追问，直接推荐
- 追问的目的是区分相似维度（比如「决策」是商业决策还是人生决策？）

**示例对话**（展示诊断节奏）：

```
使用者：我总觉得自己做决定太慢，想来想去最后还是选错

女娲：你说的决策主要是哪种场景？比如商业/投资决策，还是职业/人生方向的选择？

使用者：主要是商业上的，比如要不要做某个产品、要不要接某个合作

女娲：明白了，你的核心需求是「在資訊不完整时快速做出高质量的商业判断」。
我推荐3个候选：
[展示候选推荐...]
```
...
- 使用者的需求指向某种具体的思考方式 → 人物Skill（蒸馏某个人的思维框架）
- 使用者的需求指向某个领域的方法论 → 主题Skill（综合多人视角，见「特殊场景 > 主题Skill」）
- 不确定 → 推荐中同时包含两种类型，让使用者选

**来源A：本地已有Skill**
扫描 `.claude/skills/*-perspective/` 目录，读取每个SKILL.md的description，匹配使用者需求。已有Skill可以即插即用，不需要重新蒸馏。如果扫描结果为空（使用者还没有任何perspective skill），跳过此步，只从来源B推荐。

**来源B：新蒸馏候选**
基于需求维度表中的「思维框架方向」列，匹配最相关的人物或主题。推荐时说清楚：这个人的哪个思维框架能解决使用者的具体问题。
...
**完成检查**（自动执行）：
- [ ] 目录已创建
- [ ] 如果是中国人物：資訊源策略切换为B站原始视频/小宇宙播客/权威中文媒体优先（知乎和微資訊公众号始终排除，见資訊源黑名单）
- [ ] 如果是更新模式：已读取现有SKILL.md，标注哪些資訊需要刷新
- [ ] 如果使用者提供了本地语料：将素材复制/移动到 `sources/` 对应子目录，标记为**本地语料模式**
...
| 模式 | 触发条件 | 策略 |
|------|---------|------|
| **纯网络搜索**（默认） | 使用者没有提供本地素材 | 6个Agent全部走网络搜索，完整流程 |
| **本地语料优先** | 使用者提供了PDF/transcript/字幕/文章等 | 先分析本地素材，网络搜索变为补充 |
| **纯本地语料** | 使用者明确说「只用我给的素材」或蒸馏非公众人物 | 只分析本地素材，不做网络搜索 |

**本地语料优先模式的执行逻辑**：

1. **先读本地素材**：将使用者提供的文件按6个维度分类（一本书可能同时覆盖著作+对话+表达多个维度）
2. **识别資訊缺口**：本地素材覆盖了哪些维度？哪些维度缺失或薄弱？
3. **定向补充搜索**：只对缺失维度启动网络搜索Agent，已有充足本地素材的维度跳过搜索
4. **来源标记**：调研文件中明确区分「来自使用者提供素材」vs「来自网络搜索」
...
| 使用者整理的笔记 | 作为二手来源交叉参考 | 视具体内容 |

**本地语料的质量优势**：使用者手上的一手素材（尤其是完整书籍、长访谈原文）通常比网络搜索到的二手转述质量高得多。在資訊源优先级中，本地提供的一手素材排在最高权重。

---

以下是6个Agent的标准任务分配（纯网络搜索模式，或本地语料模式中缺失维度的补充搜索）：

启动6个并行subagent，每个负责不同資訊维度。

#### 6个Agent的任务分配

| Agent | 搜索目标 | 提取重点 | 输出文件 |
|-------|---------|---------|---------|
| 1 著作 | 书、长文、论文、newsletter | 反复出现的核心论点（≥3次=真資訊念）、自创术语、推荐书单 | `01-writings.md` |
...
- 视频字幕获取（已提供脚本，直接调用）：
  ...
  - 使用者提供本地视频文件（无字幕）：用 gemini-video skill 转写
...
Phase 1启动前，**主动扫描 `.claude/skills/` 目录**，检查是否有可用于資訊获取的skill。如果有，在调研中优先调用，比WebSearch更稳定高效：

| 已安装Skill | 用途 | 调用场景 |
|------------|------|---------|
| `gemini-video` | 分析本地视频文件，提取transcript | 使用者提供了视频文件但没有字幕 |
| `web-article-reader` | 精确读取网页文章全文 | 找到重要文章URL时，精确提取而非依赖搜索摘要 |
| `agent-reach` | 多渠道資訊获取（17个平台） | 需要从X/Reddit/YouTube等平台获取資訊 |
| `huashu-research` | 结构化深度调研 | 需要对某个维度做深度调研而非广撒网 |
| `pdf` | 读取PDF书籍/论文 | 使用者提供了PDF格式的一手素材 |
...
- **知乎**：洗稿严重、資訊失真率高，不作为任何维度的来源
- **微資訊公众号**：封闭生态、无法验证、大量二手转述，不作为来源
- **百度百科/百度知道**：資訊陈旧且不可靠
...
- **单个Agent超时**（搜索5分钟无有价值结果）：不等待，继续推进。在Phase 2中标注「資訊不足」，在诚实边界中说明
- **資訊源匮乏**（<10条可用来源）：Phase 0.5就提醒使用者，降低期望（心智模型减至2-3个），增加诚实边界篇幅
- **Agent结果冲突**：保留矛盾——矛盾本身是有价值的資訊号。用「内在张力」section收录
...
使用者确认OK → 进入Phase 3构建。
使用者觉得某个模型不对或缺少 → 回到Phase 2调整后再继续。
...
**為什麼需要这个段落**：让人物不只是「说得像」，还「做得像」。没有这个段落，人物Skill遇到需要事实的问题时会凭训练语料编造，而不是像真人一样先做功课再发言。这是人物Skill从「鹦鹉学舌」升级为「可靠思维顾问」的关键。
...
使用者看到的不是调研报告，而是[人物名]基于真实資訊做出的判断。
...
| 费曼 | 第一性原理、对权威的怀疑 | → 看基本物理/数学约束、看官方说法的逻辑漏洞、看实验資料 |
| 塔勒布 | 反脆弱、尾部风险、知识的僭妄 | → 看极端情况、看谁在承担尾部风险、看专家预测的历史记录 |
| MrBeast | 注意力工程、测试迭代 | → 看竞品資料（播放/互动）、看标题/缩略图的A/B测试空间、看受众画像 |

**关键约束**：
- 研究维度必须来自心智模型，不能是通用的「搜索相关資訊」
- 每个维度要有具体的搜索指引（搜什么、看什么資料），不能只是抽象描述
...
**主Agent综合两份报告，应用不冲突的改进，展示变更摘要请使用者确认。**
...
### 中国人物 vs 西方人物
- **中国人物**：B站原始视频/演讲、小宇宙播客、权威媒体采访（36氪/晚点/财新/极客公园）、本人著作/微博。知乎和微資訊公众号永远排除
- **西方人物**：Twitter、YouTube、Podcast、Amazon书评

### 冷门人物（公开資訊极少）
当Phase 0.5评估后发现可用来源<10条时：
1. 在Phase 0.5就告知使用者「这个人的公开資訊很少，生成的Skill质量会受限」
2. 心智模型减至2-3个，每个都标注「基于有限資訊推测」
3. 诚实边界section加大篇幅，明确列出「哪些维度資訊不足」
4. 如果使用者能提供一手素材（书籍、内部录音、私資訊），优先使用

### 蒸馏使用者自己
当使用者说「蒸馏我自己」「帮我做一个我的skill」时：
1. 女娲无法从公开渠道搜到使用者的思维框架，需要使用者提供素材
2. 引导使用者提供：个人文章/博客、录制过的视频/播客、写过的决策备忘录、自我描述
3. Phase 1的6个Agent改为分析使用者提供的素材，而非网络搜索
4. 特别注意「自我认知偏差」——使用者可能高估某些特质、忽略盲点，可以追问身边人的评价
