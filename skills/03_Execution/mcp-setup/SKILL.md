...
3. 授予所需權限（repo, issues, pull_requests 等）
...
**前置需求：**
- Docker Desktop 已安裝並運行（版本需支援 MCP Toolkit）
...
# 2. 將 Docker MCP Toolkit 連接至 Gemini（全域模式）
...
docker mcp secret --help       # 管理 API 金鑰與密鑰
```
...
2. **找到正確的指令或套件**：確認官方文件中的啟動方式（docker / npx / python 等）
