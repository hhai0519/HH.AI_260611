...
class ImageEnhancerPipeline:
    ...
    def upscale(self, scale: float = 2.0, method: str = 'lanczos') -> 'ImageEnhancerPipeline':
        ...
        method 選項：
          - 'lanczos': 最高品質，適合照片和圖標（預設）
          - 'bicubic': 速度較快，適合截圖
          - 'nearest': 像素藝術風格（保留鋸齒感）
        """
        w, h = self.img.size
...
| **掃描文件（OCR 前處理）** | `upscale(1.5)`, `contrast(1.5)`, 轉灰階, `sharpen(2.0)` |
| **產品截圖（文件用）** | `upscale(1.5)`, `sharpen(1.2)`, `brightness(1.05)` |
...
- `pdf`：圖片增強後嵌入 PDF 文件
- `artifacts-builder`：作為 Web 組件的高清貼圖
