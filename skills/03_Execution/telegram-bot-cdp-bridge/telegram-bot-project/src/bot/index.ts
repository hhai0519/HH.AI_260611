...
import { sanitizeDlp } from '../utils/dlpSanitizer';
...
export const startBot = async (cliLogLevel?: LogLevel) => {
    ...
    // 攔截使用者訊息并記錄日誌 (含 DLP 敏感過濾)
    bot.use(async (ctx, next) => {
        if (ctx.message && ctx.message.text) {
            logTelegramChat(ctx.from?.id || 'unknown', ctx.from?.username || 'unknown', ctx.message.text, false);
        }
        await next();
    });

    // V13 壓力優化：巨型報表冷卻時間 (5秒 Anti-DoS)
    let lastReportTime = 0;
    const REPORT_COOLDOWN_MS = 5000;

    // 使用 grammY Transformer 攔截發送內容 (V13: 包含 'text' in payload 安全型別防禦與滾動式報表清理)
    bot.api.config.use(async (prev, method, payload, signal) => {
        if (method === 'sendMessage' && payload && 'text' in payload && typeof payload.text === 'string') {
            const modelName = process.env.AGENT_MODEL_NAME || 'Unknown Model';
            const prefix = `[${modelName}] 萬能總管`;
            
            if (payload.text.length > 3500) {
                const now = Date.now();
                if (now - lastReportTime < REPORT_COOLDOWN_MS) {
                    payload.text = `${prefix}\n⚠️ 巨型報表發送過於頻繁，已被系統防禦阻斷。請稍候再試。`;
                    return prev(method, payload, signal);
                }
                lastReportTime = now;

                const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
                const desktopDir = path.join(os.homedir(), 'Desktop', 'Line對話紀錄', 'TG_萬能總管');
                if (!fs.existsSync(desktopDir)) fs.mkdirSync(desktopDir, { recursive: true });

                const safeCategory = "Telegram_Report";
                const safeBrief = "巨型對話紀錄";
                
                const items = fs.readdirSync(desktopDir, { withFileTypes: true });
                let maxQ = 0;
                ...
                for (const item of items) {
                    if (item.isDirectory()) {
                        ...
                        if (match) {
                            ...
                            if (match[2] === safeCategory) matchedQDir = item.name;
                        }
                    }
                }
                let threadDirName;
                if (matchedQDir) {
                    ...
                    fs.mkdirSync(path.join(desktopDir, threadDirName));
                }
                const threadDirPath = path.join(desktopDir, threadDirName);
                ...
                const currentSeqStr = String(maxSeq + 1).padStart(3, '0');
                const fileName = `${currentSeqStr}_${safeCategory}_${safeBrief}_${timestamp}.txt`;
                const reportPath = path.join(threadDirPath, fileName);
                
                try {
                    fs.writeFileSync(reportPath, sanitizeDlp(payload.text), 'utf8');
                    payload.text = `${prefix}\n⚠️ 偵測到巨型報表，已安全過濾並歸檔至：\n${reportPath}`;
                } catch (e) {
                    console.error('[Fail-safe] 無法歸檔至文字檔:', e);
                    payload.text = `${prefix}\n⚠️ 偵測到巨型報表且歸檔失敗，系統已自動截斷前3000字：\n\n${payload.text.substring(0, 3000)}...`;
                    return prev(method, payload, signal);
                }
            ...
            logTelegramChat((payload as any).chat_id, 'BOT', payload.text, true);
        }
        return prev(method, payload, signal);
    });
