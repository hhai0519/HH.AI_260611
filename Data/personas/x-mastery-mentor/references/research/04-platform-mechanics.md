...
> 调研日期：2026-04-06
> 資料时效：覆盖2023年首次开源至2026年1月第二次开源的完整演变
> 資訊分级：🟢 官方公布/开源代码可查 | 🟡 权威媒体报道/資料分析 | 🔴 社区测试推测
...
使用者可以用自然语言指令调整Feed，例如输入「Show me more tech innovations, less politics」。这是Grok直接嵌入推荐引擎的产物。
...
| **个人主页点击** | +12.0 | **24x** | 使用者点进你的主页并点赞或回复 |
| **对话深入点击** | +11.0 | **22x** | 使用者点进对话并回复或点赞 |
| **停留时间（Dwell > 2min）** | +10.0 | **20x** | 使用者点进对话并停留超过2分钟 |
...
- 早期社区分析引用的「Reply 27x, Retweet 40x」等資料来自2023版本的简化计算
...
| **屏蔽/静音/Show Less** | -74x | 大幅降低对该使用者的推荐 |
...
| **多于2个Hashtag** | 触达降低约40%，被判定为spam資訊号 |
...
- Premium回复在热门帖子讨论中默认排位更高（Q1 2026資料显示高30-40%回复曝光）
...
- 来源：[Circleboom](https://blog-content.circleboom.com/does-x-premium-boost-algorithm/) | [posteverywhere.ai](https://posteverywhere.ai/blog/how-the-x-twitter-algorithm-works) | [Buffer資料](https://buffer.com/resources/data-best-content-format-social-media/)
...
| 資料来源 | 结论 |
|----------|------|
| Buffer 2025-2026資料 | 文字帖中位互动率（0.48%）略高于视频 |
| 多家SEO/营销机构 | 原生视频获得约10x更多互动+算法偏好分发 |
| 2026社媒策略报告 | 短视频（37%）和文字（36%）使用者偏好几乎持平 |
...
| **原生视频（<2:20）** | 获得分发加权，完播率是关键資訊号 |
| **图片帖** | 增加停留时间（dwell time），正面資訊号 |
...
- 使用者在你的帖子/对话上停留超过2分钟 = +10权重（约20x Like）
...
⚠️ 以上为全球英文使用者資料。中文创作者需根据目标受众时区调整（如面向中国读者，对应北京时间约 9PM-2AM EST）。
...
| 类型 | 表现 |
|------|------|
| **Search Suggestion Ban** | 使用者名不出现在搜索建议中 |
...
- 在线工具：[shadowban.yuzurisa.com](https://shadowban.yuzurisa.com/) 输入使用者名即可检测4种限制
- 人工验证：让不关注你的人搜索你的使用者名或查找你的回复
...
- 下降主要来自**使用者行为改变**：看到Note后使用者减少转发和点赞
...
- **文字内容友好**：不像其他平台逼你做视频
- **对话驱动**：真正奖励深度交流而非表面互动
- **Promptable Feeds**：使用者可自定义推荐，意味着高质量垂直内容有长尾价值
