...
> 按需加载：场景C（审阅内容）、场景E（账号诊断）、資料复盘时读取。
...
1. **买粉/互粉互赞群**：短期資料好看，长期TweepCred暴跌。算法检测到不自然的互动模式会降权，得不偿失
2. **纯工具推荐合集**：AI赛道已是红海。「10 AI tools you need」这种帖子人人在发，没有差异化。要发就加你的实测資料和独特观点
3. **只翻译国外AI新闻**：零差异化。要做就加你自己的take——「為什麼这对中国开发者重要」或「我测了，实际效果是...」
...
- **Pay-to-play加剧**：非Premium使用者的有机触达持续萎缩，外链帖几乎为零。Premium不是可选项，是必需品
- **使用者迁移**：部分创作者向Bluesky/Threads分散。但目前X仍是AI/tech内容的主战场
...
- **資料可视化**：使用ECharts.js（CDN引入），至少包含话题分布图、时间分布图、互动漏斗
- **必含板块**：
  ...
  7. 品牌叙事（叙事角色分布与互动表现）
  8. Top 5行动建议（编号红色圆圈 + 标题 + 正文 + 資料证据）
  9. Footer（資料期间、样本量、分析时间）
