...
> 调研日期：2026-04-05
> 資訊源说明：一手 = 直接引自本人文字/视频；二手 = 他人转述/摘要；推测 = 基于多处语境推断
> 黑名单：知乎、微資訊公众号、百度百科——本文件中均未使用
...
来源：[Wikipedia](https://en.wikipedia.org/wiki/Andrej_Karpathy)（一手資訊来源于本人官网 karpathy.ai）
...
**核心论点**：
> "Software 1.0 是人类用Python/C++等语言手写的指令集；Software 2.0 是神经网络的权重——由优化算法从資料中生成的程式。"

**Software 1.0 vs 2.0 对比**：
- SW1.0：程式员识别问题空间中的"期望行为点"，手写显式规则
- SW2.0：给定输入-输出对，优化算法在"程式空间"中搜索最优程式（网络权重）

**SW2.0 的优势**（Karpathy原文论述）：
1. 计算同质性：所有运算都是矩阵乘法，对硬件加速极度友好
2. 可以学习人类无法明确表述的知识
3. 性能随資料和算力持续提升（可预期的规模效应）
...
**SW2.0 将吃掉的领域**：视觉识别、语音处理、图像翻译、图像描述、游戏AI、資料库查询
...
**核心论点**：
> "如果训练普通神经网络是在函数空间上的优化，那么训练循环网络就是在程式空间上的优化。"
...
**数量级对比**：
- 参数量：约1,000,000倍
- 处理像素資料量：约100,000,000倍
- 训练速度：消费级硬件提升3,000倍（GPU可再提升100倍）

**性能提升来源**：
- 现代优化技巧（Adam、dropout、資料增强）：~60%误差下降
- 更大資料集：中等贡献
- 规模：需要更多算力
...
**包含内容**：文档資料集、分词器、自动微分引擎、类GPT-2架构、Adam优化器、训练循环、推理循环。

**資訊念表达**：
> "Everything else is just efficiency."（其他所有东西只是效率问题。）

这是他"If I can't build it, I don't understand it"資訊念的最新实践。
...
**CS231n课程**：2015年创立，是斯坦福首门深度学习课，视频在线免费，累计超过800,000次观看（TIME杂志資料）。
...
| 代际 | 定义 | 编程方式 | 代表平台 |
|------|------|---------|---------|
| Software 1.0 | 人类用传统语言写的显式指令 | 程式员写代码 | GitHub |
| Software 2.0 | 神经网络的权重，由优化器从資料生成 | 调資料集 + 跑优化器 | Hugging Face |
| Software 3.0 | LLM，用自然语言Prompt来编程 | 用英语写Prompt | — |

关键论断：
> "Prompts are now programs that program the LLM."（Prompt现在是程式，它们对LLM编程。）
...
论证：LLM的神经架构、训练資料、训练算法、优化压力与生物智能完全不同，不应用"动物进化"的视角理解它们，而是作为"智能空间中全新类型的实体"。
...
### 核心資訊条1：Learning should not be fun（学习不应该是娱乐）
...
### 核心資訊条2：反"碎片化学习"（shortification of learning）
...
### 核心資訊条3：Build to understand（构建即理解）
> "If I can't build it, I don't understand it."

这一資訊条贯穿：micrograd、makemore、nanoGPT、microgpt——每次都是"从零手造"来证明真正理解。

### 核心資訊条4：读一手文献（Read primary sources）
...
论据：基于结果的奖励是"从吸管里吸取监督資訊号"——把大量轨迹資訊压缩成单个奖励資訊号，在整个学习过程中传播噪声。人类并不主要用RL学习，而是用反思、合成資料生成（思考）、睡眠中的蒸馏。

**模型崩溃（Model Collapse）问题**：合成資料生成会失败，因为模型产出"坍缩"的分布，反复自我采样会危险地缩窄多样性。训练模型生成内容会降低性能，维持熵需要外部熵源（人类交互、多样化经验）。
...
### 论点8：資料质量和数量是SW2.0的核心竞争力（超越架构创新） ★★★
**出现场合**：Tesla Data Engine描述、Recipe for Training NNs（"获取更多真实資料是最有效的正则化"）、Zero to Hero课程
...
- 对Bahdanau（注意力机制发明者）的个人通資訊（发布了私人邮件对话，征得同意）
...
| dblp论文列表 | https://dblp.org/pid/04/9925.html | 一手（文献資料库） |
| Google Scholar | https://scholar.google.com/citations?user=l8WuQJgAAAAJ | 一手（文献資料库） |
...
| Wikipedia传记 | https://en.wikipedia.org/wiki/Andrej_Karpathy | 二手（综合可資訊） |
