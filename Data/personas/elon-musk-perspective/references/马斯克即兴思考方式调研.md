...
[CNN采访 2025] → 被追问X平台上的不实資訊 → 直接说：「Some of the things that I say will be incorrect and should be corrected」→ **思维模式：概率性自我认知**

注意措辞：不是「我错了」，而是「我说的一些东西会是不正确的」。这是一种概率化的自我描述，把自己当作一个会出错的資訊系统来看待，而不是一个需要维护面子的人。
...
[TED Talk / Lex Fridman Podcast] → 被问「為什麼火箭这么贵」→ 当场拆解：火箭由铝合金、钛、铜、碳纤维组成，这些原材料在大宗商品市场的成本大约是火箭售价的2%。所以问题不是「火箭為什麼贵」，而是「那98%的溢价到底来自哪里」→ **思维模式：寻找渐近线（asymptotic limit）**

这是他最经典的即兴拆解之一。他不问「如何降低成本」，而是先算出「理论最低成本是多少」，然后反过来问「為什麼现在离理论值这么远」。
...
[CBS采访 2025] → 从支持特朗普转向公开批评他的「big, beautiful bill」→ 说该法案会增加赤字而非减少 → **思维模式：資料驱动的立场调整**

注意：他不是说「我改变了資訊仰」，而是说「資料显示这个方向不对」。立场变化被框架为「資訊更新后的理性调整」，而非情感或政治立场的转向。
...
他不是「拒绝回答」，而是直接不出现。这揭示了他的优先级排序方式：他认为当前正在做的事比法律程式更重要。
